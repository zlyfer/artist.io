...
var { dictionaries } = require("../config/dictionaries.json");
var { colorPalette } = require("../config/colorPalette.json");
    ...
    if (already_joined == false) {
      ...
      if (user) {
        ...
        socket.emit(
          "room_joined",
          users[user.id],
          dictionaries[rooms[user.room].language],
          colorPalette,
          rooms[user.room].dictionaries
        );
  ...
  socket.on("send_message", data => {
    if (
      ...
      let score = Math.floor(
        rooms[data.author.room].timer / (rooms[data.author.room].maxTimer / 100) -
          rooms[data.author.room].guessedIt.length
      );
